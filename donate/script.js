...
function radioBtnFunc() {
  ...
  secTwoRadio.addEventListener("click", (e) => {
    if (e.target.matches("input")) {
      ...
      const radioVal = e.target.nextElementSibling.textContent
        .match(/\d/g)
        .join("");
...
function inputFunc() {
  function elementFunc(e, input) {
    e.forEach((element) => {
      element.children[0].classList.contains("radio__active")
        ? element.children[0].classList.remove("radio__active")
        : "do nothing";
      element.children[1].classList.contains("highlight")
        ? element.children[1].classList.remove("highlight")
        : "do nothing";
    });
...
function highlightRemFunc(e) {
  for (const iterator of e.currentTarget.children) {
    iterator.children[0].classList.contains("highlight")
      ? iterator.children[0].classList.remove("highlight")
      : "do nothing";
  }
}
function navFunc() {
